        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            setupForm('documents', ['title', 'description', 'file']);
            setupForm('financial', ['title', 'period', 'description', 'file']);
            setupForm('partnerships', ['title', 'organization', 'description', 'start', 'file']);
            ...
            // Configurar drag and drop para todos os file uploads
            setupDragAndDrop();
        ...
        function showAlert(type, message, isError = false) {
            const alertElement = document.getElementById(isError ? `alert-${type}` : `success-${type}`);
            if (alertElement) {
        ...
        function setupForm(type, fields) {
            ...
            form.addEventListener('submit', function(e) {
                ...
                if (!validation.isValid) {
                    showAlert(type, `Por favor, corrija os seguintes campos: ${validation.errors.join(', ')}`, true);
                ...
                if (type === 'audits') {
                    ...
                    if (formData.status === 'approved' && formData.type === 'externa' && !formData.title.toLowerCase().includes('externa')) {
                        showAlert(type, 'Atenção: Verifique se o título condiz com o tipo de auditoria externa aprovada.', true);
                        return;
                    }
                }
                ...
                if (type === 'financial') {
                    ...
                    if (!periodRegex.test(formData.period)) {
                        showAlert(type, 'O período deve incluir ano ou especificar trimestre/semestre (ex: "1º Trimestre 2024").', true);
                        return;
                    }
                }
                ...
                showAlert(type, getSuccessMessage(type, formData));
        ...
        function editItem(type, id) {
            ...
            const formPrefix = type.slice(0, -1); // Remove 's' final
        ...
        function deleteItem(type, id) {
            ...
            if (confirm(confirmMessage)) {
                ...
                showAlert(type, `Item "${item.title}" foi excluído com sucesso.`);
            }
        }
